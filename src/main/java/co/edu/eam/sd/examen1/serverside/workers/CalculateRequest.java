...
public class CalculateRequest implements Runnable {
	...
	/**
	 * Function to perform the request. this method is called by the concurrent
	 * method.
	 *
	 * @param command
	 * @param payload
	...
	public double executeCommand(String command, String payload) throws Exception {
		...
		for (int i = 0; i < blockCount; i++) {
			...
			// TODO: agregar al pool la instancia del calculador y agregar el futuro al
			// arreglo de futuros.
			Future<Double> futuro = pool.submit(calculatorUtil);
			results.add(futuro);
		}
